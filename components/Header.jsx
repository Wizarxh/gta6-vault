    ...
    <header className="sticky top-0 z-40 border-b border-vc-border bg-gradient-to-r from-black via-black to-black/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-2.5 sm:px-6">
        ...
        <button 
          ...
          className="md:hidden p-2 rounded-md text-vc-muted hover:text-vc-cyan hover:bg-vc-cyan/5 transition-all"
      ...
      {/* Mobile Navigation */}
      {isMobileMenuOpen && (
        <nav className="md:hidden border-t border-vc-border/50 bg-black/95 backdrop-blur px-4 py-3">
          <div className="flex flex-col gap-2">
            ...
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="px-3 py-2 rounded-md font-mono text-xs uppercase tracking-[0.25em] text-vc-muted hover:text-vc-cyan hover:bg-vc-cyan/5 transition-all text-left w-full"
            >
              Language: {currentLang.toUpperCase()}
            </button>
            {isOpen && (
              <div className="border border-vc-border rounded-md bg-black/50 backdrop-blur p-1">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    onClick={() => {
