...
export const articles = [
  ...
  },
  {
    ...
    author: "Marcus Quinn",
    readingTime: 7,
    hero: "linear-gradient(135deg, #00D4FF 0%, #7B2CBF 100%)",
    breaking: false,
    body: [
  ...
  },
  {
    ...
    author: "Devon Ortiz",
    readingTime: 6,
    hero: "linear-gradient(135deg, #FF6B9D 0%, #2E1A47 100%)",
    breaking: false,
    body: [
  ...
  },
  {
    ...
    author: "Léa Marchetti",
    readingTime: 4,
    hero: "linear-gradient(135deg, #FFD23F 0%, #FF6B9D 100%)",
    breaking: true,
    body: [
  ...
  },
  {
    ...
    author: "Sasha Ng",
    readingTime: 8,
    hero: "linear-gradient(135deg, #7B2CBF 0%, #00D4FF 100%)",
    breaking: false,
    body: [
...
function getPublishedArticles() {
  try {
    ...
    return raw.map((a) => ({
      ...
      status: a.tag || STATUS.VERIFIED,
      category: a.category || "Analysis",
      date: a.publishedAt ? a.publishedAt.slice(0, 10) : new Date().toISOString().slice(0, 10),
      author: a.author || "GTA6 Vault",
      readingTime: Math.max(3, Math.ceil((a.body || "").split(" ").length / 200)),
      hero: a.heroImage || "linear-gradient(135deg, #FF6B9D 0%, #00D4FF 100%)",
      breaking: false,
      body: (a.body || "").split("\n\n").filter(Boolean),
      sources: [],
    }));
...
function getAllArticlesRaw() {
  const published = getPublishedArticles();
  const publishedSlugs = new Set(published.map((a) => a.slug));
  const staticFiltered = articles.filter((a) => !publishedSlugs.has(a.slug));
  return [...published, ...staticFiltered];
}

export function getAllArticles() {
  return getAllArticlesRaw().sort((a, b) => (a.date < b.date ? 1 : -1));
}

export function getArticleBySlug(slug) {
  return getAllArticlesRaw().find((a) => a.slug === slug);
}
